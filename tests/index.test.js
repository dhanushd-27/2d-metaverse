...
describe("Websockets tests", () => {
    ...
    test('Correct move should be populated to others users',async () => {
        ws1.send(JSON.stringify({
            "type": "move",
            "payload": {
                "x": adminX + 1,
        ...
        message1 = await waitAndGetLatestMessages(ws2Messages);
        expect(message1.type).toBe("movement");
        expect(message1.payload.x).toBe(adminX + 1);
        expect(message1.payload.y).toBe(adminY);
        expect(message1.userId).toBe(adminId);
    });
    ...
    test('User Left', async () => {
        ...
        const message = await waitAndGetLatestMessages(ws2Messages);
